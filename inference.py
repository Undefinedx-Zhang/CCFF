...
def main():
    ...
    elif backbone == 'ResNet50_CD_Cross_sup_without_attention':
        model = models.FPA_ResNet50_CD_sup_without_attention(num_classes=num_classes, conf=config['model'],
                                                             testing=True)
    elif backbone == 'ResNet50_CD_sup_without_attention':
        model = models.FPA_ResNet50_CD_sup_without_attention(num_classes=num_classes, conf=config['model'],
                                                             testing=True)
    print(f'\n{model}\n')
    ...
    for index, data in enumerate(tbar):
        ...
        tbar.set_description(
            'Test Results | IoU(change): {:.4f}, F1: {:.4f} , Kappa: {:.4f}, TPR: {:.4f}, TNR: {:.4f}, OA: {:.4f}, Recall: {:.4f}|'. \
            format(IoU[1], F1, Kappa, TPR, TNR, OA, R))
...
def parse_arguments():
    ...
    parser.add_argument('--model', default='Output/best_model.pth',
                        type=str, help='Path to the trained .pth model')
    parser.add_argument('--save', action='store_true', help='Save images')
    parser.add_argument('--Dataset_Path', default="E:\\sci_file\\Data\\WHU-CD-256", type=str,
