...
class Trainer(BaseTrainer):
    def __init__(self, model, resume, config, supervised_loader, unsupervised_loader, iter_per_epoch,
        ...
        WEIGHTS = r'CTCD/pretrained/deeplabv3plus_resnet101.pth'
        self.CTCD = DeepLabV3Plus('resnet101', 15)
        self.CTCD.load_state_dict(torch.load(WEIGHTS))
        self.CTCD = self.CTCD.cuda()
        for param in self.CTCD.parameters():
    ...
    def _train_epoch(self, epoch):        
        ...
        for batch_idx in tbar:
            ...
            total_loss, cur_losses, outputs = self.model(A_l=A_l, B_l=B_l, target_l=target_l, 
                                                         WA_ul=WA_ul, WB_ul=WB_ul, SA_ul=SA_ul, SB_ul=SB_ul, CTCD=self.CTCD)
